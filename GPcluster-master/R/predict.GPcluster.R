...
predict.GPcluster <- function(object, xnew, conf.level = 0.95, conf.out = TRUE, parallel = FALSE){
  ...
  #在这里，需要输出gfun.prob,每一个k的gp预测mu.pred,var.pred
  #RMSE.m就是每一类GP的se
  #yhat.m就是每一类gp的预测值
  ...
  if(conf.out){
    ...
    return(list(yhat = mu.pred, s2=var.pred, y_ori=yhat.m,se_ori=RMSE.m,prob=gfun.prob,LCL = quantile.pred[1,], UCL = quantile.pred[2,]))
  }else{
    return(list(yhat= mu.pred, s2=var.pred,y_ori=yhat.m,se_ori=RMSE.m,prob=gfun.prob))
  }

  #在这里，需要输出gfun.prob,每一个k的gp预测mu.pred,var.pred
  #RMSE.m就是每一类GP的se 注意 需要标准化还原。
  #yhat.m就是每一类gp的预测值
}
